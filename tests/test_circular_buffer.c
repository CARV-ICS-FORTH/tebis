...
int main(void)
{
	...
	while (i < NUM_OF_ALLOCATIONS) {
		...
		if (op_code == ALLOCATION_IS_SUCCESSFULL) {
			++i;
			if (addr != last_allocation + last_size && addr != memory_region) {
				log_fatal("No the memory address expected");
				_exit(EXIT_FAILURE);
			}
			...
			log_debug("Not enough space at the tail allocation number %d size %lu resetting buffer", i,
				  (long unsigned)size);
			reset_circular_buffer(c);
		} else if (op_code == SPACE_NOT_READY_YET) {
			log_debug("Space at the tail not freed yet allocation number %d size %lu \n", i,
				  (long unsigned)size);
			//idx = rand() % NUM_OF_ALLOCATIONS;
