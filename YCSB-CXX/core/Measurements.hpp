...
class OperationStatistics {
	...
	void print(Op o, std::ostream &of)
	{
		of << "[TAIL][" << Op2Str[o] << "][usec]"
		   << " min " << min << " max " << max << " avg " << avg << " samples " << total_samples << " missing "
		   << missing << " lat50 " << lat50 << " lat70 " << lat70 << " lat90 " << lat90 << " lat99 " << lat99
		   << " lat999 " << lat999 << " lat9999 " << lat9999 << std::endl;
	}
};
...
class Measurements {
	...
	void printStatistics(std::ostream &of)
	{
		for (int op = LOAD; op < MAXOPS; op++) {
			...
			if (sum.total_samples > 0) {
				...
				sum.print(static_cast<Op>(op), of);
			}
		}
	}
};
