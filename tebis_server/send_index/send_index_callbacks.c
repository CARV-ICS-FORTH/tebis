...
static void send_index_allocate_rdma_buffer_in_replicas(struct send_index_context *context, uint32_t level_id)
{
	...
	for (uint32_t i = 0; i < r_desc->region->num_of_backup; ++i) {
		...
		msg_header *req_header = msg_pair.request;
		send_index_fill_get_rdma_buffer_request(req_header, r_desc, level_id);
...
static void send_index_close_compaction(struct send_index_context *context, uint32_t level_id)
{
	...
	for (uint32_t i = 0; i < r_desc->region->num_of_backup; ++i) {
		...
		// allocate the request msg pair, retry until success
		struct sc_msg_pair msg_pair = sc_allocate_rpc_pair(r_conn, send_index_close_compaction_request_size,
								   send_index_close_compaction_reply_size,
								   CLOSE_COMPACTION_REQUEST);

		while (msg_pair.stat != ALLOCATION_IS_SUCCESSFULL) {
			msg_pair = sc_allocate_rpc_pair(r_conn, send_index_close_compaction_request_size,
							send_index_close_compaction_reply_size,
							CLOSE_COMPACTION_REQUEST);
		}

		msg_header *req_header = msg_pair.request;
		send_index_fill_close_compaction_request(req_header, r_desc, level_id);
		send_index_fill_reply_fields(&msg_pair, r_conn);
		msg_pair.request->session_id = (uint64_t)r_desc->region;
...
static void send_index_replicate_index_segment(struct send_index_context *context, char *buf, uint32_t buf_size)
{
	...
	memcpy(r_desc->local_buffer->addr, buf, buf_size);
	for (uint32_t i = 0; i < r_desc->region->num_of_backup; ++i) {
		struct connection_rdma *r_conn = sc_get_data_conn(server, r_desc->region->backups[i].kreon_ds_hostname);
		...
		while (1) {
			ret = rdma_post_write(r_conn->rdma_cm_id, NULL, r_desc->local_buffer->addr, buf_size,
					      ...
					      (uint64_t)r_desc->remote_mem_buf[i].addr, r_desc->remote_mem_buf[i].rkey);
			if (!ret)
				break;
		}
	}
...
static void send_index_swap_levels(struct send_index_context *context, uint32_t level_id)
{
	...
	for (uint32_t i = 0; i < r_desc->region->num_of_backup; ++i) {
		...
		// allocate the request msg pair, retry until success
		struct sc_msg_pair msg_pair = sc_allocate_rpc_pair(r_conn, send_index_swap_levels_request_size,
								   send_index_swap_levels_reply_size,
								   REPLICA_INDEX_SWAP_LEVELS_REQUEST);

		while (msg_pair.stat != ALLOCATION_IS_SUCCESSFULL) {
			msg_pair = sc_allocate_rpc_pair(r_conn, send_index_swap_levels_request_size,
							send_index_swap_levels_reply_size,
							REPLICA_INDEX_SWAP_LEVELS_REQUEST);
		}

		msg_header *req_header = msg_pair.request;
		send_index_fill_swap_levels_request(req_header, r_desc, level_id);
...
void send_index_compaction_started_callback(void *context, uint32_t src_level_id)
{
	...
	send_index_allocate_rdma_buffer_in_replicas(send_index_cxt, src_level_id);
