...
static void handle_task(struct krm_server_desc *mydesc, struct krm_work_task *task)
{
	...
	case REPLICA_INDEX_FLUSH_REQ: {
		...
		if (f_req->is_last) {
			...
			// log_info("Index transfer ended deregistering buffers for level %d",
			// f_req->level_id);
			/*now free src index level and dst and replica then mark new index as valid*/
			if (f_req->level_id > 1) {
		...
		*(uint32_t *)((uint64_t)task->reply_msg + actual_reply_size + (padding - TU_TAIL_SIZE)) =
		...
		log_info("REPLICA: Master wants to flush seg id %d  DONE!", f_req->seg_id);
		break;
	}
	case GET_LOG_BUFFER_REQ: {
		void *addr;
